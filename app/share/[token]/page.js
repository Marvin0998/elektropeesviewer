...
// Kunden-Ansicht: Nur lesen, kein Login nötig
    ...
    <>
      ...
      <nav className="navbar">
        ...
        <div style={{
          ...
          color: '#22c55e', padding: '5px 12px', borderRadius: 20, fontSize: 12, fontWeight: 500
        }}>
          👁 Nur-Lesen Ansicht
        </div>
      </nav>
      ...
      <main className="page">
        ...
        <div style={{ marginBottom: 32 }}>
          ...
          {project.description && (
            <p style={{ color: 'var(--text-muted)', marginTop: 6 }}>{project.description}</p>
          )}
          <p style={{ fontSize: 13, color: 'var(--text-muted)', marginTop: 8 }}>
            {(photos || []).length} Fotos · Geteilt von Elektro Pees
          </p>
        </div>
        ...
        {(!photos || photos.length === 0) ? (
          ...
          <div className="photo-grid">
            {photos.map(photo => (
              <Link
                key={photo.id}
                href={`/share/${token}/viewer/${photo.id}`}
