...
export default function ShareViewerPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const { data: link } = await supabase
        .from('share_links').select('project_id').eq('token', token).single()
      if (!link) { setNotFound(true); setLoading(false); return }

      // Projekt
      const { data: proj } = await supabase
        .from('projects').select('name').eq('id', link.project_id).single()
      setProjectName(proj?.name || '')
      setProjectId(link.project_id)

      // Foto prüfen ob es zum Projekt gehört
      const { data: ph } = await supabase
        .from('photos').select('*').eq('id', photoId).eq('project_id', link.project_id).single()
      if (!ph) { setNotFound(true); setLoading(false); return }
      setPhoto(ph)

      const [{ data: anns }, { data: nts }] = await Promise.all([
        supabase.from('annotations').select('*').eq('photo_id', photoId),
        supabase.from('notes').select('*').eq('photo_id', photoId)
      ])
      setAnnotations(anns || [])
      setNotes(nts || [])
      setLoading(false)
  ...
  function geodesicToPolyline(y1,p1,y2,p2){
    ...
    for(let i=0;i<=SEGMENTS;i++){const v=slerp(va,vb,i/SEGMENTS),pt=fromVec(v),s=worldToScreen(pt.yaw,pt.pitch);if(s){cur.push(s)}else{if(cur.length>1)segs.push(cur);cur=[]}}
  ...
  function renderLine(item) {
    ...
    function MidLabel({pt}){if(!label||!pt)return null;const w=label.length*8+18;return<g><rect x={pt.x-w/2} y={pt.y-13} width={w} height={22} rx={5} fill="rgba(0,0,0,0.82)"/><text x={pt.x} y={pt.y+4} textAnchor="middle" fill={color} fontSize={13} fontWeight={700} fontFamily="Inter,sans-serif">{label}</text></g>}
    return(
      <g key={id}>
        {segments.map((seg,i)=><polyline key={i} points={ptsToStr(seg)} fill="none" stroke={color} strokeWidth={2} strokeLinecap="round" strokeLinejoin="round"/>)}
        ...
        {type==='text'&&start&&<g><rect x={start.x-4} y={start.y-20} width={(label||'').length*9+14} height={26} rx={5} fill="rgba(0,0,0,0.8)"/><text x={start.x+3} y={start.y+2} fill={color} fontSize={16} fontWeight={700} fontFamily="Inter,sans-serif">{label}</text></g>}
      </g>
    )
  }
