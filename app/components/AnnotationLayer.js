// components/AnnotationLayer.js
// Gemeinsamer SVG-Renderer für Admin- und Kunden-Viewer
// Wird in beiden Viewern identisch verwendet → nie wieder Doppel-Rendering
...
// Rendert eine einzelne Annotation als SVG-Gruppe
export function AnnShape({ item, isPreview=false, selected=false, onClick, onDoubleClick }) {
  ...
  function MidLabel({pt}){
    ...
    return <g>
      ...
      <text x={pt.x} y={pt.y+4} textAnchor="middle" fill={color} fontSize={13} fontWeight={700} fontFamily="Inter,sans-serif">{label}</text>
    </g>
  }
  ...
  if (type==='rect') {
    ...
    return <g {...props}>
      ...
      {label && <g>
        <rect x={cx-label.length*4-6} y={cy-13} width={label.length*8+12} height={22} rx={5} fill="rgba(0,0,0,0.82)"/>
        <text x={cx} y={cy+4} textAnchor="middle" fill={color} fontSize={13} fontWeight={700} fontFamily="Inter,sans-serif">{label}</text>
      </g>}
    </g>
  }

  // TEXT — nur am Startpunkt, kein Linienstrich
  if (type==='text') {
    ...
    return <g {...props}>
      ...
      {selected && <text x={start.x} y={start.y-fontSize-14} fill="rgba(255,255,255,0.7)" fontSize={10} fontFamily="Inter,sans-serif">✎ Doppelklick = bearbeiten · Rechtsklick = verschieben</text>}
    </g>
  }

  // LINIEN (measure, arrow, line)
